...
async function fetchMealData(meal) {
    try {
...
function updateOrderList() {
    ...
    if (orders.length === 0) {
        ...
        orders.forEach((meal, index) => {
            ...
            orderName.textContent = `${meal.orderNumber}. ${meal.strMeal}`;
...
function completeOrder() {
    ...
    if (incompleteOrders.length === 0) {
        alert('No incomplete orders to complete.');
    } else {
        const orderNumber = prompt('Enter the order number to mark as complete (or enter 0 to cancel):');
        
        if (orderNumber === '0') {
            alert('Order completion canceled.');
        } else {
            const order = incompleteOrders.find(order => order.orderNumber === parseInt(orderNumber));
